...
<div class="container-fluid py-5">
  <div class="glass-card p-4 mx-auto animate__animated animate__fadeInUp" style="max-width:1400px;">
    <h2 class="gradient-text mb-4 text-white">Manage Applications</h2>
      ...
      <table class="table table-dark align-middle glass-card animate__animated animate__fadeIn"
        ...
        <tbody>
          ...
          <tr style="background: rgba(0,0,0,0.55); border-radius: 1rem; color: #fff; transition: background 0.2s;">
            ...
            <td class="align-middle"><span class="badge bg-<?= $app['status'] === 'accepted' ? 'success' : ($app['status'] === 'rejected' ? 'danger' : 'warning') ?> text-white fw-bold" style="font-size:1rem;"> <?= ucfirst($app['status']) ?> </span></td>
            <td class="text-white align-middle"> <?= htmlspecialchars($app['comment']) ?> </td>
            <td class="align-middle">
              <div class="d-flex gap-2">
                <a href="?controller=admin&action=viewApp&id=<?= $app['id'] ?>" class="btn btn-sm btn-glass btn-outline-warning text-white" data-bs-toggle="tooltip" title="View details" style="border-color: #ffe259; color: #ffe259;"><i class="bi bi-eye"></i></a>
                <a href="?controller=admin&action=updateApp&id=<?= $app['id'] ?>" class="btn btn-sm btn-glass btn-outline-secondary text-white" data-bs-toggle="tooltip" title="Edit application" style="border-color: #aaa; color: #fff;"><i class="bi bi-pencil"></i></a>
                <button type="button" class="btn btn-sm btn-glass btn-outline-danger text-white" data-bs-toggle="modal" data-bs-target="#deleteAppModal<?= $app['id'] ?>" title="Delete application" style="border-color: #ef4444; color: #ef4444;"><i class="bi bi-trash"></i></button>
                <!-- Modal -->
                  ...
                  <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content glass-card animate__animated animate__zoomIn">
                      <div class="modal-header border-0">
                        <h5 class="modal-title gradient-text" id="deleteAppModalLabel<?= $app['id'] ?>">Confirm Delete</h5>
